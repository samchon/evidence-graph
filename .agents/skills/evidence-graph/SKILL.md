...
- **The unit model decides what merges, and it is narrower than the rules'.** A class is not a type unit, so `class Sale` beside `namespace Sale` is one declaration of `Sale` here while `evidence/singular` and `evidence/documented` treat the pair as one identity.
